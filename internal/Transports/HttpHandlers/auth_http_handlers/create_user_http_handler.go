...
func init() {
	HttpHandlers.RegisterHttpHandler(MakeCreateUserHttpHandler())
}

func MakeCreateUserHttpHandler() HttpHandlers.RegisterableHttpHandler {
	return func(router *mux.Router, appCtx AppContext.Context, services Services.Bundle) {
		api := router.PathPrefix("/auth").Subrouter()

		endpoint := AuthenticationEndpoints.MakeCreateUserEndpoint(appCtx, services)
		decoder, _ := MakeCreateUserRequestDecoder(appCtx)
		encoder, _ := MakeCreateUserResponseEncoder(appCtx)

		api.Methods("POST").Path("/user").Handler(kithttp.NewServer(
			endpoint,
			decoder,
			encoder,
		))
...
func MakeCreateUserRequestDecoder(appCtx AppContext.Context) (kithttp.DecodeRequestFunc, error) {
	return func(ctx context.Context, r *http.Request) (interface{}, error) {
		var Req authenticator.CreateUserRequest
...
func MakeCreateUserResponseEncoder(appCtx AppContext.Context) (kithttp.EncodeResponseFunc, error) {
	return func(ctx context.Context, httpResponse http.ResponseWriter, endpointResponse interface{}) error {
		// DECODE RESPONSE
		resp, ok := endpointResponse.(*authenticator.CreateUserResponse)
		if ! ok {
